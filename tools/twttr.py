...
class TWTTR(object):
    ...
    def __init__(self):
        ...
        self.TWIT_PIPE = None
        self.NET = nx.DiGraph()
        self.APP_NAME = 'CESS-twitterAPI'
        self.CONSUMER_KEY = 'dumCXzkfYBTSam4fnkLsYW5GO'
        self.CONSUMER_SECRET = 'jm6hxs3xCfijp5LGU4JovQ69bdfiJYWrsr31N9LCk2mmWTjIcQ'
        self.ACCESS_TOKEN = '1441665122-Z5IElTNaI4hD1Yh48Em3dhM69iW6TAIgzm4aHkn'
        self.ACCESS_TOKEN_SECRET = 'LuWbrEpQH7reOV7ahia19wXTgf4gCAkWd3eTYytB3geav'
    ...
    def login(self, CONSUMER_KEY=None,
                        CONSUMER_SECRET=None,
                        ACCESS_TOKEN=None,
                        ACCESS_TOKEN_SECRET=None):
        if any([CONSUMER_KEY==None, ACCESS_TOKEN==None]):
            self.TWIT_PIPE = twitter.Api(self.CONSUMER_KEY,
                                                            self.CONSUMER_SECRET,
                                                            self.ACCESS_TOKEN,
                                                            self.ACCESS_TOKEN_SECRET)
        else:
            self.TWIT_PIPE = twitter.Api(CONSUMER_KEY,
                                                            CONSUMER_SECRET,
                                                            ACCESS_TOKEN,
                                                            ACCESS_TOKEN_SECRET)
    ...
    def getUsersTweets(self,name, count=200):
        ...
        for tw in tweets:
            ...
            print("Times Fav'd:\t", i["fav_count"])
            print("Is Retweet:\t", i["isRT"])
            if i["isRT"]:
                print("\tOriginal poster:\t",i["orig_poster_sn"])
                print("\tTimes original RT'd:\t", i["orig_rt_count"])
                print("\tTimes original Fav'd:\t", i["orig_fav_count"])
            cleaned.append(i)
    ...
    def cleanTweet(self,tweet):
        ...
        if "retweeted_status" in tw.keys():
            isRT = True
            try:
                cleaned["orig_rt_count"] = tw["retweeted_status"]["retweet_count"]
            except:
                cleaned["orig_rt_count"] = 0
            try:
                cleaned["orig_fav_count"] = tw["retweeted_status"]["favorite_count"]
            except:
                cleaned["orig_fav_count"] =0

            cleaned["orig_poster_sn"] = tw["retweeted_status"]["user"]['screen_name']
        ...
        try:
            cleaned["fav_count"] = int(tw["favorite_count"])
        except:
            cleaned["fav_count"] = 0
        ...
        try:
            if cleaned["rt_count"] == cleaned['orig_rt_count']:
                cleaned["rt_count"] = 0
        except:
            pass
        ...
        for i in retweets:
            ...
            for j in docs_to_update:
                ...
                try:    #print(out)
                    ...
                    count += 1
                    url = "visualization/"+str(htmlname)+".html"
                    #print("new url",url)
                    w = ix.writer()
    ...
    def graph_Tweets(self,results,saveAs):
        ...
        file = open("./visualization/"+saveAs+".html", "w+")
        lines = form.readlines()
        for ln in lines:
